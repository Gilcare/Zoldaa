...
async def add_domain_glossary(agent: p.Agent) -> None:
    await agent.create_term(
        name = "Contact MyDoc",
        description= "The phone number of a doctor, at +1234567890",
    )

    await agent.create_term(
        name="Consultation Times",
        description="Consultation hours are Sunday to Saturday, 7 AM to 2AM",
    )

    await agent.create_term(
        name="Jake Sullivan",
        synonyms=["Dr Strange"],
        description="Renowned neurologist"
    )
...
async def create_scheduling_journey(server: p.Server, agent: p.Agent) -> p.Journey:
   ...
   # Add edge case guideline
   await journey.create_guideline(condition="The patient says their visit is urgent",
...
async def initialize_parlant() -> None:
    nlp_service_instance = LiteLLMService(
        model=LITELLM_PROVIDER_MODEL_NAME,
